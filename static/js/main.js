document.addEventListener("DOMContentLoaded", function () {
  ...
  async function loadBreedsDropdown() {
    try {
      ...
      dropdown.innerHTML =
        '<option value="" disabled selected>Select a Breed</option>'; // Reset dropdown
  ...
  async function displayBreedDetails(breed) {
    ...
    try {
      ...
      slideshow.innerHTML = images
        .map(
          (img) =>
            `<img src="${img.url}" alt="${breed.name}" class="slideshow-img">`
        )
        .join("");
      startSlideshow();
  ...
  async function loadFavorites() {
    try {
      ...
      if (favorites.length === 0) {
        container.innerHTML =
          '<p class="no-favorites">No favorites yet. Start adding some cats!</p>';
        return;
      }

      container.innerHTML = favorites
        .map((fav) => {
          //checks for empty img url and replaces with a placeholder.
            const imageUrl =
            fav.image && fav.image.url
              ? fav.image.url
              : "/static/img/placeholder--cat.png";
          return `
                    <div class="favorite-card" data-id="${fav.id}">
                        <img src="${imageUrl}" alt="Favorite cat">
  ...
  async function loadVoteHistory() {
    try {
      ...
      if (!votes || votes.length === 0) {
        container.innerHTML =
          '<p class="no-history">No voting history available.</p>';
        return;
      }
